...
class SearchSpace2D final : public nanoplan::SearchSpace<State3D> {
 public:
    ...
    for (int i = -1; i < 2; ++i) {
      for (int j = -1; j < 2; ++j) {
        for (int k = -1; k < 2; ++k) {
          ...
          if (0 <= succ.x && succ.x < 10 && 0 <= succ.y && succ.y < 10 &&
              0 <= succ.z && succ.z < 10) {
  ...
  nanoplan::Cost get_from_to_cost(const State3D& from,
                                  const State3D& to) override {
    double manhattan = std::abs(from.x - to.x) + std::abs(from.y - to.y) +
                       std::abs(from.z - to.z);
    return nanoplan::Cost(manhattan);
  }

  nanoplan::Cost get_from_to_heuristic(const State3D& from,
                                       const State3D& to) override {
