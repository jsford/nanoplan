...
class LPAStar final : public Planner<SPACE> {
    ...
    private:
        ...
        std::pair<double,double> calculate_key(const typename SPACE::state_type& state) const;
...
void LPAStar<SPACE>::update_node(const typename SPACE::state_type& state) {
    if( state != start )
    {
        ...
        for( const auto& pred : space->get_predecessors(state) )
        {
            ...
            if( pq.contains(state) ) { pq.remove(state); }
...
template <typename SPACE>
std::pair<double,double> LPAStar<SPACE>::calculate_key(const typename SPACE::state_type& state) const {
